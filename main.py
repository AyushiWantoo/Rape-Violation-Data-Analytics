...
# Load data
data_path = r"Rape Violation In INDIA.csv"
...
# Aggregate data to avoid duplicate entriesx
...
st.write("Aggregated DataFrame:", aggregated_df)

try:
    pivoted_data = aggregated_df.pivot_table(
        index='YEAR',
        columns='State',
        values='No. Of Cases In Which Offenders Were Known To The Victims',
        aggfunc='sum'
    )
    ...
    st.error(f"Error in generating line chart: {e}")

try:
    st.line_chart(pivoted_data.iloc[:, 0])  # Plot only the first column as a test
except Exception as e:
    st.error(f"Error in generating test line chart: {e}")

st.write("Pivoted DataFrame After Pivot (if successful):", pivoted_data)

aggregated_df.to_csv("aggregated_df_before_pivot.csv", index=False)
pivoted_data.to_csv("pivoted_data_after_pivot.csv", index=False)
